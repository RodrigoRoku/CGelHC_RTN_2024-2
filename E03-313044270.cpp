...
int main()
{
	...
	float	x1 = 0.0, y1 = -0.5,  z1 = 0.0 ,  //Vertice frontal derecho
			x2 = -0.5, y2 = -0.5, z2 = 0.0,  //Vértice frontal izquierdo
	...
	*/
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//Tamanio  en "x" 1.15(-.575, 0.575) -- 0.1 de espacio etre triangulos, 0.1 de separacion con la base, el centro no está en 0.0, piensa que estás empujado el borde frontal de x cuando pienses en profundidad

		color = glm::vec3(0.0f, 0.6f, 0.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); 

		// NIVEL INFERIOR ---------------------------------------------------------------------------------------------------------
		// Paso 1 Calcular dezplazamiento en x: Hay distancia de 2.0 en x 
		//		1er tríangulo: Para nivel inferiorValor en x = 0 - longitud_negra/2 = -2.0, por lo que el borde de la piramide tiene que estar en -2.0, si la longitud es de 0.575  entonces se tiene que trasladar -2 +0.575 - 0.05(P/ que sobresalga) en x 
		//		otros: incrementar el valor de x (mover a la derecha), tanteo
		// Paso 2: calcular desplazamiento en y
		//		Usar los mismos valores de Y de la cara frontal y probar
		//PASO 3: Calcular desplazamiento en z, en esta cara este es el que se calcula para cada piramide
		//		Para primer triangulo: -2 (distancia entre borde inf y el punto 0.0) - 0.2 (separacion con la piramide negra) - + 1.15/2 ( punto medio de la base de la piramide)
		//		Segundo tríandulo: -2.2 (desp. Primer triangulo) - 1.15 (long del segundo tríangulo -0.05 (separacion entre triangulos 

		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(-1.375f, -1.275f, -2.15f)); //z es como tu x de la cara frontal, moverlo solo empuja hacia adelante o hacia atrás. x es como la z tienes que moverlo Para meter la piramide
